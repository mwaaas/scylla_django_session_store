...
from scylla_sessions.sessions import (
    SessionStore as ScyllaSessionStore,
...
class SessionTestsMixin:
    ...
    def test_actual_expiry(self):
        ...
        with override_settings(SESSION_SERIALIZER='django.contrib.sessions.serializers.PickleSerializer'):
            ...
            try:
                self.session['foo'] = 'bar'
                self.session.set_expiry(timedelta(seconds=1))
                self.session.save()
                time.sleep(1)
...
class ScyllaSessionTests(SessionTestsMixin, TestCase):

    backend = ScyllaSessionStore
    ...
    def test_management_command(self):
        ...
        cluster_keyspace = connection.cluster.metadata.keyspaces
        # confirm keyspace does not exists
        assert not cluster_keyspace.get(session_keyspace)
